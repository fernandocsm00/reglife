...
/**
 * LandingHero — bloco da landing, usado pelas duas portas de entrada:
 *   /       → "fazer o teste"   (entry: "teste")
 *   /plano  → "receber o plano" (entry: "plano")
 *
 * Além do visual, é aqui que a origem do lead é guardada (porta + UTMs da
 * URL) pro DiagnosticoScreen mandar no POST /api/leads. Ver lib/leadSource.ts.
 */
...
import { rememberLeadSource, type LeadEntry } from "@/lib/leadSource";

interface Props {
  entry: LeadEntry;
  title: string;
  /** Trecho do subtítulo destacado em âmbar. */
  highlight: string;
...
export function LandingHero({
  ...
  // Guarda a origem na primeira página aberta nesta aba (só a 1ª vale).
  useEffect(() => {
    rememberLeadSource(entry, window.location.search);
  }, [entry]);

  useEffect(() => {
    const p = getStoredPlan();
    // Aluno com plano vai direto pro hub /meu-plano — a landing só serve
    // pra quem ainda não fez o nivelamento.
    if (p) {
